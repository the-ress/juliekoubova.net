...
function plugin(options) {
  ...
  function getLangAttribute(node) {
    if (node.attrs) {
      var attr = node.attrs.find(function (a) {
        return a.name == 'lang';
      });
  ...
  function hyphenateText(dom) {
    ...
    dom.childNodes = dom.childNodes.map(function(node) {
      
      if (
        node.nodeName === '#text' && 
        isPresent(options.elements, dom.tagName)
      ) {
        if (!disabled) {
      ...
      if (node.nodeName === '#comment' && node.data === 'no-hyphenate') {
        disabled = true;
        return null;
      }
